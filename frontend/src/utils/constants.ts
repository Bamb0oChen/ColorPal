...
export function checkAchievements(collectedIds: string[]): Achievement[] {
  ...
  return ALL_ACHIEVEMENTS.filter((achievement) => achievement.check(uniqueIds));
}

function hasAllColors(collectedIds: string[], requiredIds: string[]): boolean {
  return requiredIds.every((id) => collectedIds.includes(id));
}

function countByFamily(collectedIds: string[], family: ColorFamily): number {
  return ALL_COLORS.filter((color) => (
    color.family === family && collectedIds.includes(color.id)
  )).length;
}

function countByRarity(collectedIds: string[], rarity: Rarity): number {
  return ALL_COLORS.filter((color) => (
    color.rarity === rarity && collectedIds.includes(color.id)
  )).length;
}

function countWarmColors(collectedIds: string[]): number {
  return ALL_COLORS.filter((color) => (
    [ColorFamily.RED, ColorFamily.ORANGE, ColorFamily.YELLOW].includes(color.family)
    && collectedIds.includes(color.id)
  )).length;
}

function countCoolColors(collectedIds: string[]): number {
  return ALL_COLORS.filter((color) => (
    [ColorFamily.GREEN, ColorFamily.BLUE, ColorFamily.PURPLE].includes(color.family)
    && collectedIds.includes(color.id)
  )).length;
}
...
function createAchievement(
  ...
  return { id, name, description, category, color, check };
}
...
export const ALL_ACHIEVEMENTS: Achievement[] = [
  ...
  createAchievement(
    ...
    '#6C5CE7',
    (ids) => countByFamily(ids, ColorFamily.RED) === getColorsByFamily(ColorFamily.RED).length,
  ),
  createAchievement(
    'achieve_orange_collector',
    '橙色收集家',
    '集齐全部 4 种橙色系颜色',
    AchievementCategory.COLLECT,
    '#6C5CE7',
    (ids) => (
      countByFamily(ids, ColorFamily.ORANGE) === getColorsByFamily(ColorFamily.ORANGE).length
    ),
  ),
  createAchievement(
    'achieve_yellow_hunter',
    '黄色猎人',
    '集齐全部 5 种黄色系颜色',
    AchievementCategory.COLLECT,
    '#6C5CE7',
    (ids) => (
      countByFamily(ids, ColorFamily.YELLOW) === getColorsByFamily(ColorFamily.YELLOW).length
    ),
  ),
  createAchievement(
    'achieve_green_guardian',
    '绿色守护者',
    '集齐全部 6 种绿色系颜色',
    AchievementCategory.COLLECT,
    '#6C5CE7',
    (ids) => (
      countByFamily(ids, ColorFamily.GREEN) === getColorsByFamily(ColorFamily.GREEN).length
    ),
  ),
  createAchievement(
    'achieve_blue_dreamer',
    '蓝色梦想家',
    '集齐全部 6 种蓝色系颜色',
    AchievementCategory.COLLECT,
    '#6C5CE7',
    (ids) => countByFamily(ids, ColorFamily.BLUE) === getColorsByFamily(ColorFamily.BLUE).length,
  ),
  createAchievement(
    'achieve_purple_mystery',
    '紫色神秘',
    '集齐全部 5 种紫色系颜色',
    AchievementCategory.COLLECT,
    '#6C5CE7',
    (ids) => (
      countByFamily(ids, ColorFamily.PURPLE) === getColorsByFamily(ColorFamily.PURPLE).length
    ),
  ),
  createAchievement(
    'achieve_gray_master',
    ...
    AchievementCategory.COLLECT,
    '#6C5CE7',
    (ids) => countByFamily(ids, ColorFamily.GRAY) === getColorsByFamily(ColorFamily.GRAY).length,
  ),
  createAchievement(
    'achieve_full_spectrum',
    '全色谱大师',
    '集齐全部 36 种颜色',
    AchievementCategory.COLLECT,
    '#6C5CE7',
    (ids) => ids.length >= ALL_COLORS.length,
  ),
  // 稀有度类
  createAchievement(
    'achieve_novice',
    ...
    '#FFD700',
    (ids) => countByRarity(ids, Rarity.COMMON) >= 18,
  ),
  createAchievement(
    'achieve_rare_finder',
    '稀有发现者',
    ...
    AchievementCategory.RARITY,
    '#FFD700',
    (ids) => countByRarity(ids, Rarity.RARE) >= 11,
  ),
  createAchievement(
    'achieve_epic_hunter',
    '史诗猎手',
    ...
    AchievementCategory.RARITY,
    '#FFD700',
    (ids) => countByRarity(ids, Rarity.EPIC) >= 5,
  ),
  createAchievement(
    'achieve_legend_eye',
    '传说之眼',
    ...
    AchievementCategory.RARITY,
    '#FFD700',
    (ids) => countByRarity(ids, Rarity.LEGENDARY) >= 2,
  ),
  // 特殊组合
  createAchievement(
    'achieve_primary',
    ...
    '#00B894',
    (ids) => hasAllColors(ids, ['red_red', 'yellow_bright', 'blue_sky']),
  ),
  createAchievement(
    'achieve_rainbow',
    '彩虹之路',
  ...
  createAchievement(
    ...
    AchievementCategory.COMBO,
    '#00B894',
    (ids) => countWarmColors(ids) >= 5 && countCoolColors(ids) >= 5,
  ),
  createAchievement(
    'achieve_black_white',
    '黑白之间',
    ...
    AchievementCategory.COMBO,
    '#00B894',
    (ids) => hasAllColors(ids, ['gray_black', 'gray_white', 'gray_silver']),
  ),
];
